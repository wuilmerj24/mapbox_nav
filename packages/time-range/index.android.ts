    ...
    try {
      ...
      this.timeRanger.setOnDragChangeListener(
        new nl.joery.timerangepicker.TimeRangePicker.OnDragChangeListener({
          onDragStart(param0: nl.joery.timerangepicker.TimeRangePicker.Thumb) {
            ref.get()?.sendEvent(TimeRange.dragEvent, { start: null, end: null, event: param0.toString() });
            return true;
          },
          onDragStop(param0: nl.joery.timerangepicker.TimeRangePicker.Thumb) {
            ref.get()?.sendEvent(TimeRange.dragEvent, { start: null, end: null, event: param0.toString() });
          },
        })
      );
      this.timeRanger.setOnTimeChangeListener(
        new nl.joery.timerangepicker.TimeRangePicker.OnTimeChangeListener({
          onDurationChange(param0) {
            ...
            ref?.get()?.sendEvent(TimeRange.timeChangeEvent, { start: time.start.toString(), end: time.end.toString() });
          },
          onEndTimeChange(param0) {
            const time = param0;
            ref?.get()?.sendEvent(TimeRange.timeChangeEvent, { start: time.getHour().toString(), end: time.getMinute().toString() });
          },
